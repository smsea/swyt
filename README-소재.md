...
0~4단계를 전부 돌려 소재 11종(정지 9 + 영상 2)을 뽑았다. 검수 실패 0건.

받은 원본은 `원본/` 에 있는 7개(사진 4, 영상 3)로, **전부 「예약을 부르는 블로그
운영법」 단권을 손에 든 컷**이다. 「검색되는 블로그 글쓰기법」도, 두 권이 함께 잡힌
컷도, 내지 컷도 없다. 그래서 원래 배정 원칙(b2=다른 책/내지, b3=두 권 함께)을 쓸 수
없어 **배경의 밝기·개방감으로 흐름을 만들었다** — b1 실내 야간(닫힘) → b2 발코니
숲(빛이 들어옴) → b3 들판·하늘(트임).

영상 3건은 각 1.37~1.81초로 **비트당 4초에 못 미쳐 그대로 쓸 수 없었다.** 대신 정지
소재와 같은 합성 화면에 Ken Burns 를 걸어 12초를 만들었다 (`--style designed`, 기본값).
4초 이상 되는 클립이 들어오면 `cuts.json` 의 `video.file`/`start` 를 채우고
`--style raw` 로 돌리면 실사 구간이 그대로 들어간다.
