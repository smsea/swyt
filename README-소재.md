...
- **책이 헤드라인을 덮지 않게.** 알파의 행 폭이 급히 좁아지는 지점(책 아랫변과
  ...
  막는다.
- **손목을 캔버스 바닥까지.** 원본은 팔이 짧게 잘려 있어 그대로 앉히면 화면 중간에서
  끊겨 뜬다. 손목은 폭이 일정한 기둥이라 단면을 아래로 늘여 잇고, 아래로 갈수록
  어둡게 떨어뜨려 그림자 속으로 들여보낸다.
  단, **손목 기둥이 실제로 있을 때만** 잇는다 (아래쪽 대표 폭이 최대 폭의 10~60%).
  팔이 프레임 밖인 컷에서 억지로 이으면 책의 흰 띠가 아래로 번진다.

받은 4장 중 손목이 잡힌 것은 `20260704_121727` 과 `20260705_104842` 뿐이다.
`20260703_231255`(실내 야간)과 `20260704_105429`(데크)는 팔이 프레임 밖이라
자기 사진만으로는 손이 바닥에 닿지 않는다. 그래서 b1 은 **배경만 실내 야간에서
가져오고(`bg_photo`) 책·팔은 121727 에서** 쓴다. 대신 b1 과 b3 이 같은 책·손을
공유한다. 자기 사진 그대로 쓰려면 `cuts.json` 의 b1 에서 `photo` 를
`20260703_231255` 로 되돌리고 `bg_photo` 를 `null` 로 두면 된다.
